...
@section('path')
    <div class="page-header">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Daftar Relasi Formasi Tim {{ $this_year }}</li>
        </ol>
    </div>
@endsection
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mb-3 text-left">
                            ...
                            <a href="{{ route('formasi_tim.create') }}" class="btn btn-primary">Tambah
                                Data</a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped" id="dataTable">
                            ...
                            <tbody>
                                @foreach ($formasi_tim as $item)
                                    <tr>
                                        <td class="text-center">{{ $loop->iteration }}</td>
                                        <td class="text-center">{{ $item->struktur->tim->name ?? '-' }}</td>
                                        <td class="text-center">{{ $item->struktur->seksi->name ?? '-' }}</td>
                                        <td class="text-center">{{ $item->koordinator->name ?? '-' }}</td>
                                        <td class="text-center">{{ $item->anggota->name ?? '-' }}</td>
                                        <td class="text-center">{{ $item->area->pulau->name ?? '-' }}</td>
                                        <td class="text-center">
                                            <a href="{{ route('formasi_tim.edit', $item->uuid) }}"><button
            ...
            <div class="modal-content">
                <div class="modal-body p-2">
                    ...
                    <div class="px-5 pb-8 text-center mt-3">
                        <form action="{{ route('formasi_tim.destroy') }}" method="POST">
                            @csrf
                            @method('delete')
